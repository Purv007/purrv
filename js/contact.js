...
// message to Purv's inbox, without needing a custom backend server.
...
//   2. Add an Email Service (connect your Gmail: patelpurv908@gmail.com)
//   3. Create an Email Template with variables: {{from_name}}, {{from_email}}, {{message}}
//   4. Copy your Public Key, Service ID, and Template ID
//   5. Paste them into js/config.js (window.ENV)
...
// ============================================================================
(() => {
  // ---- CONFIG: reads from js/config.js (window.ENV) ----
  ...
  const IDLE_HTML = 'purv@portfolio:~$ Waiting for your message... <span class="contact__terminal-cursor"></span>';
  ...
  function showError(message) {
    status.innerHTML = '';
    status.appendChild(line('purv@portfolio:~$ send_message()'));
  ...
  async function runSendSequence(name, email, message) {
    ...
    const sendPromise = (window.emailjs && EMAILJS_PUBLIC_KEY)
    ...
    if (sent) {
      ...
      status.appendChild(line('&nbsp;'));
      status.appendChild(line('Please try again, or email me directly at patelpurv908@gmail.com', 'status-muted'));
    }
    status.appendChild(line('&nbsp;'));
    status.appendChild(line('purv@portfolio:~$ <span class="contact__terminal-cursor"></span>'));
  }
